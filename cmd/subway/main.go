...
func main() {
	...
	flag.StringVar(&saveConfigPath, "save", "", "Save config to specified file")
	...
	if saveConfigPath != "" {
		if err := SaveConfig(config, saveConfigPath); err != nil {
			fmt.Println(red(fmt.Sprintf("Failed to save config: %v", err)))
		} else {
